...
* `qa-tools` - Plugin Human-readable name, e.g. "Sample Extension"
* `Qatools` - Which namespace we will use for the plugin. Usually the plugin name or the slug in uppercase without spaces, e.g. "SampleExtension"
* `qa-tools` - Uses using dashes normally, e.g. "sample-extension"
* `__TRIBE_DOMAIN__` - Translation domain normally with "tribe-" prefix, e.g. "tec-labs-sample-extension"
* `qa_tools` - Uses underscores, so it's safe for variables, e.g. "sample_extension"
