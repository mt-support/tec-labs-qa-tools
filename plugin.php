<?php
/**
 * Plugin Name:       The Events Calendar Extension: qa-tools
 * Plugin URI:        
 * GitHub Plugin URI: https://github.com/mt-support/tec-labs-qa-tools
 * Description:       
 * Version:           1.0.1
 ...
 */
define( 'TRIBE_EXTENSION_QA_TOOLS_FILE', __FILE__ );
 ...
 * @since 1.0.0
 */
function tribe_extension_qa_tools() {
	...
	Tribe__Autoloader::instance()->register_prefix(
		'\\Tribe\\Extensions\\Qatools\\',
		__DIR__ . DIRECTORY_SEPARATOR . 'src' . DIRECTORY_SEPARATOR . 'Tec',
		'qa-tools'
	);

	// Deactivates the plugin in case of the main class didn't autoload.
	if ( ! class_exists( '\Tribe\Extensions\Qatools\Plugin' ) ) {
		tribe_transient_notice(
			'qa-tools',
			'<p>' . esc_html__( 'Couldn\'t properly load "The Events Calendar Extension: qa-tools" the extension was deactivated.', '__TRIBE_DOMAIN__' ) . '</p>',
			[],
	...
	tribe_register_provider( '\Tribe\Extensions\Qatools\Plugin' );
}

// Loads after common is already properly loaded.
add_action( 'tribe_common_loaded', 'tribe_extension_qa_tools' );
